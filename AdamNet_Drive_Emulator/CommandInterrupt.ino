void CommandInterrupt(){                                           // New byte on the AdamNet
  ...
  if (elapsedTime >= CommandTimeout){      // If the Command Timeout has been exceeded, then this should be a command
  ...
  EIFR = bit (INTF2);                      // Clear flag for any interrupts on INT2 that were triggered while we were in the ISR
}