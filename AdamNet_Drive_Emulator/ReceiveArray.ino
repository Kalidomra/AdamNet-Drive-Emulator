int ReceiveArray(byte arraytoreceive[], int lengthofarray){	       // Receive array from AdamNet
  // ==================== Read all of the bytes required into the array =======================
  for (int j=0; j<=lengthofarray ;j++){
    byte incomingbyte= 0;
// ===============  Wait for Start bit ===============
    while ((PIND & _BV(PD0)) == 0){        // Wait for PD0 to go HIGH
    }
...
// ===============  Loop Through and read the bits =============== 
    for (byte i=8; i > 0; --i){
      ...
      __asm__("nop\n\t""nop\n\t""nop\n\t""nop\n\t""nop\n\t""nop\n\t""nop\n\t");
      if ((PIND & _BV(PD0))== 0){          // If PD0 is LOW then this is a "1" bit
