void SDCommand(byte transferbytes[], byte devicenumber){
  ...
  if (commandin == 0xF1){                  // READ - Return the SD Card Current Directory
    ...
    Serial.print("Adam Command: SD Card Directory Block: ");
  ...
  if (commandin == 0xF2){                  // READ - Return Currently Mounted Disk
    ...
    Serial.print("Adam Command: Current Disk on D");
  ...
  if (commandin == 0xF3){                  // READ - Mount/Unmount Disk
    ...
    if (data2 == 0 && data3 ==0){
      ...
      Serial.print("Adam Command: Unmount D");
      Serial.println(devicenumber - 3);
      ...
      Serial.print("Adam Command: Mount D");
      Serial.print(devicenumber - 3);
      Serial.print(": ");
      Serial.println(GetFileName(filesindex[MountedFile[devicenumber-4]]));
      //loadedblock[devicenumber-4] = 0xFFFFFFFF;        // We don't have a block loaded for the new file
      StatusSetup(0x40,devicenumber);                 // Set the status to 'disk in"
    }
    EepromStringWrite((devicenumber * 100) + 2, GetFileName(filesindex[MountedFile[devicenumber-4]]));
    EEPROM.write(devicenumber*100, data3);
    EEPROM.write((devicenumber*100)+1, data2);
    refreshscreen = 1;
  }
  if (commandin == 0xF4){                  // WRITE - Write to LCD
    // data1 = not used, data2 = not used, data3 = 1 = Refresh
    if (data3 ==1){
      Serial.println("Adam Command: Refresh LCD");
      refreshscreen=1;
    }
    else{
      Serial.println("Adam Command: Writing to the LCD :");
      lcd.clear();
      lcd.setCursor(0,0);
      for(int i =3; i<=18; i++){
  ...
  if (commandin == 0xFA){                  // READ - Format Disk (Can be used if regular 0xFACE is disabled)
    // data1 = not used, data2 = not used , data3 = not used
    Serial.print("Adam Command: Format D");
    Serial.println(devicenumber - 3);       
